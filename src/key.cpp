 ...
 * included.
 *
 * seckey must point to an output buffer of length at least CKey::PRIVATE_KEY_SIZE bytes.
...
bool CKey::SetPrivKey(const CPrivKey &seckey, bool fCompressedIn) {
    if (!ec_seckey_import_der(secp256k1_context_sign, (unsigned char*)begin(), &seckey[0], seckey.size()))
        return false;
    fCompressed = fCompressedIn;
    fValid = true;
    return true;
...
CPrivKey CKey::GetPrivKey() const {
    ...
    seckeylen = PRIVATE_KEY_SIZE;
    ret = ec_seckey_export_der(secp256k1_context_sign, (unsigned char*)&seckey[0], &seckeylen, begin(), fCompressed ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED);
    assert(ret);
...
bool CKey::Derive(CKey& keyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
    ...
    bool ret = secp256k1_ec_seckey_tweak_add(secp256k1_context_sign, (unsigned char*)keyChild.begin(), out);
    UnlockObject(out);
