...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    bool fVerbose = false;
    if (params.size() > 1)
        fVerbose = (params[1].getInt<int>() != 0);
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 3 && !params[3].isNull()) {
        if (Params().GetConsensus().NetworkUpgradeActive(nextBlockHeight, Consensus::UPGRADE_OVERWINTER)) {
            int64_t nExpiryHeight = params[3].getInt<int64_t>();
    ...
    for (size_t idx = 0; idx < inputs.size(); idx++) {
        ...
        if (sequenceObj.isNum())
            nSequence = sequenceObj.getInt<int>();
        ...
        for (size_t idx = 0; idx < prevTxs.size(); idx++) {
...

            {
                ...
                if (prevOut.exists("amount")) {
                    coins->vout[nOut].nValue = AmountFromValue(prevOut.find_value("amount"));
                }
            }
...
UniValue sendrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        bool fMissingInputs;
        if (!AcceptToMemoryPool(mempool, state, tx, false, &fMissingInputs, &txFeeRate, nMaxRawTxFee)) {
            if (state.IsInvalid()) {
