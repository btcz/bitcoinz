...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"       (string, required) The BitcoinZ address to send to.\n"
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinZ address");
    }
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"       (string, required) The BitcoinZ address for transactions.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "                           Those recipients will receive less BitcoinZ than you enter in their corresponding amount field.\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; BitcoinZ server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    for (size_t idx = 0; idx < outputs.size(); idx++) {
        ...
        string txid = o.find_value("txid").get_str();
        if (!IsHex(txid))
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, expected hex txid");

        int nOutput = o.find_value("vout").getInt<int>();
        if (nOutput < 0)
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BitcoinZ address: ") + input.get_str());
            }
...
UniValue zc_benchmark(const UniValue& params, bool fHelp)
{
    ...
    int samplecount = params[1].getInt<int>();
    ...
    for (int i = 0; i < samplecount; i++) {
        if (benchmarktype == "sleep") {
            ...
            if (params.size() < 3) {
                ...
                int nThreads = params[2].getInt<int>();
            ...
            int nInputs = 11130;
            if (params.size() >= 3) {
                nInputs = params[2].getInt<int>();
            }
            sample_times.push_back(benchmark_large_tx(nInputs));
        } else if (benchmarktype == "trydecryptnotes") {
            int nKeys = params[2].getInt<int>();
...
UniValue z_viewtransaction(const UniValue& params, bool fHelp)
{
    ...
    for (size_t i = 0; i < wtx.vShieldedSpend.size(); ++i) {
        ...
        auto decrypted = wtxPrev.DecryptSaplingNote(op).value();
        auto notePt = decrypted.first;
        auto pa = decrypted.second;
...
UniValue z_getoperationstatus_IMPL(const UniValue& params, bool fRemoveFinishedOperations=false)
{
    ...
    std::sort(arrTmp.begin(), arrTmp.end(), [](UniValue a, UniValue b) -> bool {
        const int64_t t1 = a.get_obj().find_value("creation_time").getInt<int64_t>();
        const int64_t t2 = b.get_obj().find_value("creation_time").getInt<int64_t>();
        return t1 < t2;
    });
...
UniValue z_sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        string address = o.find_value("address").get_str();
        bool isZaddr = false;
        ...
        if (!IsValidDestination(taddr)) {
            ...
            if (IsValidPaymentAddress(res)) {
                ...
                if (containsSproutOutput && containsSaplingOutput) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot send to both Sprout and Sapling addresses using z_sendmany");
                }

                // If sending between shielded addresses, they must be the same type
                if ((fromSprout && toSapling) || (fromSapling && toSprout)) {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot send between Sprout and Sapling addresses using z_sendmany");
                }
    ...
    unsigned int nAnchorDepth = std::min((unsigned int) nMinDepth, nAnchorConfirmations);

    // Create operation and add to global queue
    std::shared_ptr<AsyncRPCQueue> q = getAsyncRPCQueue();
    std::shared_ptr<AsyncRPCOperation> operation( new AsyncRPCOperation_sendmany(builder, contextualTx, fromaddress, taddrRecipients, zaddrRecipients, nMinDepth, nAnchorDepth, nFee, contextInfo) );
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 3) {
        nUTXOLimit = params[3].getInt<int>();
        if (nUTXOLimit < 0) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Limit on maximum number of UTXOs cannot be negative");
        }
    }
    ...
    if (params.size() > 4) {
        int nNoteLimit = params[4].getInt<int>();
        if (nNoteLimit < 0) {
    ...
    if (useAnySprout || useAnySapling || zaddrs.size() > 0) {
        ...
        std::vector<SproutNoteEntry> sproutEntries;
        std::vector<SaplingNoteEntry> saplingEntries;
        pwalletMain->GetFilteredNotes(sproutEntries, saplingEntries, zaddrs, nAnchorConfirmations);
        ...
        for (const SproutNoteEntry& entry : sproutEntries) {
            ...
            if (!maxedOutNotesFlag) {
                ...
                size_t increase = (sproutNoteInputs.empty() && !isToSproutZaddr) || (sproutNoteInputs.size() % 2 == 0) ? JOINSPLIT_SIZE(SAPLING_TX_VERSION) : 0;
                if (estimatedTxSize + increase >= max_tx_size ||
...
void OnWalletRPCPreCommand(const CRPCCommand& cmd)
{
    ...
    if (!cmd.okSafeMode && IsInitialBlockDownload(Params())) {
