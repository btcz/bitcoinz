...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
      ...
      ImportAddress(dest, strLabel);
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinZ address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
  if (fPruneMode)
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid BitcoinZ spending key
        if (fImportZKeys) {
    ...
    while (pindex && pindex->pprev && pindex->GetBlockTime() > nTimeBegin - 7200) { //TIMESTAMP_WINDOW =7200
        pindex = pindex->pprev;
    }
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by BitcoinZ %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
