...
Backing up your BitcoinZ private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
These instructions are specific for the officially supported BitcoinZ Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`bitcoinz.conf` located in the data directory which is `~/.bitcoinz/` unless it's been overridden with `datadir=` setting):
...
Note that bitcoinzd will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`bitcoinz-cli backupwallet <nameofbackup>`.
...
If you generate a new BitcoinZ address, it will not be reflected in the backup file.
...
`bitcoinz-cli z_exportwallet <nameofbackup>`
...
`bitcoinz-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`bitcoinz-cli z_exportkey <z-address>`
...
`bitcoinz-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`bitcoinz-cli z_importkey <z-priv-key>`
...
`bitcoinz-cli help importprivkey`
