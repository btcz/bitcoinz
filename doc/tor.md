...
1. Run BitcoinZ behind a Tor proxy
-------------------------------

The first step is running BitcoinZ behind a Tor proxy. This will already make all
...
config file):
	...
	HiddenServiceDir /var/lib/tor/bitcoinz-service/
...
for normal IPv4/IPv6 communication, use:
	...
	./bitcoinzd -onion=127.0.0.1:9050 -externalip=zctestseie6wxgio.onion -discover
...
configured. For cookie authentication the user running bitcoinzd must have write access 
to the `CookieAuthFile` specified in Tor configuration. In some cases this is 
preconfigured and the creation of a hidden service is automatic. If permission problems 
are seen with `-debug=tor` they can be resolved by adding both the user running tor and 
the user running bitcoinzd to the same group and setting permissions appropriately. On 
Debian-based systems the user running bitcoinzd can be added to the debian-tor group, 
...
Now use bitcoinz-cli to verify there is only a single peer connection.

	bitcoinz-cli getpeerinfo
...
To connect to multiple Tor nodes, use:
	...
	./bitcoinzd -onion=127.0.0.1:9050 -addnode=zctestseie6wxgio.onion -dnsseed=0 -onlynet=onion
