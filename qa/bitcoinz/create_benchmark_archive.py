...
BITCOINZ_CLI = './src/bitcoinz-cli'
USAGE = """
Requirements:
- find
- xz
- %s (edit BITCOINZ_CLI in this script to alter the path)
- A running mainnet bitcoinzd using the default datadir with -txindex=1
...
LD_LIBRARY_PATH=src/leveldb python qa/bitcoinz/create_benchmark_archive.py
""" % BITCOINZ_CLI

def check_deps():
    if subprocess.call(['which', 'find', 'xz', BITCOINZ_CLI], stdout=subprocess.PIPE):
        print USAGE
        sys.exit()
