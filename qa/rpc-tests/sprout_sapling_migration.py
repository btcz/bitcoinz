...
def check_migration_status(node, destination_address, migration_state):
    ...
    assert_equal(destination_address, status['destination_address'])	    assert_true(migration_state in ALL_MIGRATION_STATES, "Unexpected migration state %r" % migration_state)
    assert_equal(non_zero_unmigrated_amount, Decimal(status['unmigrated_amount']) > Decimal('0.00'))
    assert_equal(non_zero_unfinalized_migrated_amount, Decimal(status['unfinalized_migrated_amount']) > Decimal('0'))	    expected_enabled = migration_state not in [DISABLED_NO_FUNDS, DISABLED_BEFORE_MIGRATION]
    assert_equal(non_zero_finalized_migrated_amount, Decimal(status['finalized_migrated_amount']) > Decimal('0'))	    expected_sprout_funds = migration_state in [DISABLED_BEFORE_MIGRATION, ENABLED_BEFORE_MIGRATION]
    assert_equal(finalized_migration_transactions, status['finalized_migration_transactions'])	    positive_unfinalized_amount = migration_state == DURING_MIGRATION
    assert_equal(len_migration_txids, len(status['migration_txids']))	    positive_finalized_amount = migration_state == AFTER_MIGRATION
