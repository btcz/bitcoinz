...
class WalletTreeStateTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        # Spend coinbase utxos to create three notes of 10 BTCZ minus default fee each
        recipients = []
        recipients.append({"address": myzaddr, "amount": Decimal('12500.0') - DEFAULT_FEE})
        ...
        assert_equal(Decimal(resp), (Decimal('12500.0') - DEFAULT_FEE) * 3)
        ...
        recipients = []
        recipients.append({"address": self.nodes[2].z_getnewaddress(), "amount": Decimal('12500.0') - DEFAULT_FEE})
        myopid = self.nodes[0].z_sendmany(mytaddr, recipients)
        wait_and_assert_operationid_status(self.nodes[0], myopid)
