...
class WalletTreeStateTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        assert_equal(Decimal(resp), Decimal('12499.9999') * 3 )
        ...
        recipients = []
        recipients.append({"address":self.nodes[2].z_getnewaddress(), "amount":Decimal('12500.0') - Decimal('0.0001')})
        myopid = self.nodes[0].z_sendmany(mytaddr, recipients)
        wait_and_assert_operationid_status(self.nodes[0], myopid)
        ...
        recipients = []
        recipients.append({"address":self.nodes[2].z_getnewaddress(), "amount":Decimal('26000')})
        recipients.append({"address":self.nodes[2].z_getnewaddress(), "amount":Decimal('11499.9997') - Decimal('0.0001')})
