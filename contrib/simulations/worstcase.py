...
def collect_times():
    # Find bench_bitcoinz binary
    script_dir = os.path.dirname(os.path.realpath(__file__))
    base_dir = os.path.dirname(os.path.dirname(script_dir))
    bench_bitcoinz = os.path.join(base_dir, 'src', 'bench', 'bench_bitcoinz')

    # Run bench_bitcoinz binary
    try:
        result = subprocess.run([bench_bitcoinz], stdout=subprocess.PIPE, universal_newlines=True)
        result.check_returncode()
        result = result.stdout
    except AttributeError:
        # Use the older API
        result = subprocess.check_output([bench_bitcoinz], universal_newlines=True)
