#!/usr/bin/env bash
# Copyright (c) 2024 The BitcoinZ Community
...
(
    ...
    # Setup the directory where our BitcoinZ build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built BitcoinZ to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        case "$HOST" in
            ...
            *)
                ...
                {
                    find "${DISTNAME}/bin" -type f -executable -not -name "*.sh" -print0
        ...
        # copy over the example bitcoinz.conf file. if contrib/devtools/gen-bitcoinz-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/bitcoinz.conf" "${DISTNAME}/"
