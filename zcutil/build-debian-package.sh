...
PACKAGE_VERSION=$($SRC_PATH/src/bitcoinzd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/bitcoinzd $DEB_BIN
cp $SRC_PATH/src/bitcoinz-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/bitcoinzd.1 $DEB_MAN
cp $SRC_DOC/man/bitcoinz-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/bitcoinzd.bash-completion $DEB_CMP/bitcoinzd
cp $SRC_PATH/contrib/bitcoinz-cli.bash-completion $DEB_CMP/bitcoinz-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/bitcoinzd.1
gzip --best -n $DEB_MAN/bitcoinz-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/bitcoinzd $DEB_BIN/bitcoinz-cli
